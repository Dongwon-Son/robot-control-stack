...
struct SimAdaptor {
  ...
  static inline bool read_array(std::ifstream& in, float* dst, size_t count) {
    in.read(reinterpret_cast<char*>(dst), static_cast<std::streamsize>(count * sizeof(float)));
    return in.good();
  }

  static bool load_dense(std::ifstream& in, Dense& d) {
  ...
  static bool load_layernorm(std::ifstream& in, LayerNorm& ln) {
  ...
  static bool load_block(std::ifstream& in, SimAdaptorBlock& blk) {
 ...
 private:
  ...
  static bool load_norm_stats_stream(std::ifstream& in,
                                     size_t float_count,
                                     int dof,
                                     int history_steps,
                                     NormStats& stats) {
